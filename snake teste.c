...
int main ()
{
	...
	setfillstyle(1,1);
	bar(s-s/2,s-s/2,(s/2)+s*(1350/s),s+s/2); //topo
	bar(s-s/2,(-s/2)+s*(700/s),(s/2)+s*(1350/s),(s/2)+s*(700/s));	//para baixo
	bar(s-s/2,s-s/2,s+s/2,(s/2)+s*(700/s)); //esquerda
	bar((-s/2)+s*(1350/s),s-s/2,(s/2)+s*(1350/s),(s/2)+s*(700/s)); //direta
	...
	for(i=1;i<l;i++)
		...
		while(1)
		{
			...
			if(GetAsyncKeyState(VK_RIGHT)&&d!=0)
				...
				d=3;
			if(getpixel(X[0],Y[0])==2) //condição se a comida for pega
			{
				rx=s; ry=s;
				setfillstyle(1,2);
				while(getpixel(rx,ry)!=0)
